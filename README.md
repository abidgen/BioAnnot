...
so editing the pathway synonym map costs no API calls (see [Resume cache](#resume-cache)).
...
| `FORCE_REMERGE` | Optional | Bypass the **final** layer only; replay merge + enrich from the raw cache (zero API cost). Forces that replay even when nothing changed — synonym/reference edits already trigger it automatically via `full_key` (default `false`) | — |
...
|---|---|---|---|
| Raw extraction | `cache/raw/{gene}_{extract_key}.json` | gene, disease context, extraction model, PubMed depth, extraction-prompt version | gene/source/disease/extraction-model/prompt change — **not** synonym or reference edits |
| Final enriched | `cache/final/{gene}_{full_key}.json` | `extract_key` + merge model + synonym-file hash + Reactome-file hash + enrich params | any of the above **or** any synonym/reference/merge/enrich change |
...
- `FORCE_REMERGE=true` — bypass the **final** layer only; replay merge + enrich from the raw
  cache and rewrite the final cache. **Zero API cost** when the raw cache is warm.

**Curate pathways with zero API cost.** Because `full_key` includes the synonym-file hash but
...
with the new synonyms and **no fetch or extraction API calls**:
...
#    replays merge + enrich from the raw cache (zero API cost). FORCE_REMERGE=true
#    forces the same path even when nothing changed.
python pipeline.py
```
...
- **The raw cache key intentionally excludes synonym/reference content.** `make_extract_key`
  ...
  into `make_extract_key`: it would couple the layers and silently break zero-cost re-merge
